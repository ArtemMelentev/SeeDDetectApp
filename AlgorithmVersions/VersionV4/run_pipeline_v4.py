...
RESULTS_TABLE_COLUMNS = ["Image Name", "Lab Result", "SeedDetect Result", "delta"]
...
def _refine_quad_by_border_color(img_bgr: np.ndarray, quad: np.ndarray) -> tuple[np.ndarray, dict]:
    ...
    each side is a colour-profile problem, then map the refined edge lines
    ...
    paper_like = (light >= l_min) & (sat <= s_max) & (chroma <= c_max)
    edge_mask = paper_like.astype(np.uint8) * 255
    edge_k = max(3, min(9, odd(min(out_w, out_h) * 0.003, 3)))
    edge_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (edge_k, edge_k))
    edge_mask = cv2.morphologyEx(edge_mask, cv2.MORPH_CLOSE, edge_kernel, iterations=1)
    edge_bool = edge_mask > 0

    def _first_edge_run(profile: np.ndarray, max_scan: int, reverse: bool = False) -> int | None:
        scan = profile[::-1] if reverse else profile
        scan = scan[:max_scan].astype(np.float32)
        if scan.size == 0:
            return None
        run = max(4, min(18, int(round(scan.size * 0.06))))
        if scan.size < run:
            return None
        support = _moving_average_1d(scan, run)
        hits = np.where(support >= 0.62)[0]
        if hits.size == 0:
            return None
        pos = max(0, int(hits[0] - run // 2))
        return int(profile.size - 1 - pos) if reverse else int(pos)

    def _collect_horizontal_edges(side: str) -> tuple[np.ndarray, int]:
        step = max(1, int(round(out_w / 500.0)))
        band = max(3, min(11, odd(out_w * 0.004, 3)))
        half = band // 2
        max_scan = max(max_y, int(round(out_h * 0.24)))
        pts: list[tuple[float, float]] = []
        total = 0
        for x in range(0, out_w, step):
            x0 = max(0, x - half)
            x1 = min(out_w, x + half + 1)
            profile = edge_bool[:, x0:x1].mean(axis=1)
            y = _first_edge_run(profile, max_scan, reverse=(side == "bottom"))
            total += 1
            if y is not None:
                pts.append((float(x), float(y)))
        return np.asarray(pts, dtype=np.float32), total

    def _collect_vertical_edges(side: str) -> tuple[np.ndarray, int]:
        step = max(1, int(round(out_h / 500.0)))
        band = max(3, min(11, odd(out_h * 0.004, 3)))
        half = band // 2
        max_scan = max(max_x, int(round(out_w * 0.24)))
        pts: list[tuple[float, float]] = []
        total = 0
        for y in range(0, out_h, step):
            y0 = max(0, y - half)
            y1 = min(out_h, y + half + 1)
            profile = edge_bool[y0:y1, :].mean(axis=0)
            x = _first_edge_run(profile, max_scan, reverse=(side == "right"))
            total += 1
            if x is not None:
                pts.append((float(x), float(y)))
        return np.asarray(pts, dtype=np.float32), total

    def _robust_fit_line(values: np.ndarray, coords: np.ndarray, min_pts: int) -> tuple[float, float, float] | None:
        if values.size < min_pts:
            return None
        keep = np.ones(values.shape[0], dtype=bool)
        m = 0.0
        b0 = float(np.median(values))
        for _ in range(4):
            if int(keep.sum()) < min_pts:
                return None
            m, b0 = np.polyfit(coords[keep], values[keep], 1)
            resid = values - (m * coords + b0)
            med = float(np.median(resid[keep]))
            mad = float(np.median(np.abs(resid[keep] - med))) + 1e-6
            limit = max(4.0, 4.5 * 1.4826 * mad)
        ...
        if int(keep.sum()) < min_pts:
            return None
        m, b0 = np.polyfit(coords[keep], values[keep], 1)
        resid = values[keep] - (m * coords[keep] + b0)
        rmse = float(np.sqrt(np.mean(resid * resid))) if resid.size else 0.0
        return float(m), float(b0), rmse

    def _fit_horizontal_edge(side: str) -> tuple[float, float, float] | None:
        pts, total = _collect_horizontal_edges(side)
        min_pts = max(24, int(round(total * 0.20)))
        fit = _robust_fit_line(pts[:, 1], pts[:, 0], min_pts) if pts.size else None
        if fit is None:
            return None
        m, b0, rmse = fit
        if abs(m) > 0.25 or rmse > max(8.0, out_h * 0.025):
            return None
        y0 = b0
        y1 = m * float(out_w - 1) + b0
        if side == "top":
            if min(y0, y1) < -out_h * 0.04 or max(y0, y1) > out_h * 0.28:
                return None
        else:
            if min(y0, y1) < out_h * 0.72 or max(y0, y1) > out_h * 1.04:
                return None
        return m, -1.0, b0

    def _fit_vertical_edge(side: str) -> tuple[float, float, float] | None:
        pts, total = _collect_vertical_edges(side)
        min_pts = max(24, int(round(total * 0.20)))
        fit = _robust_fit_line(pts[:, 0], pts[:, 1], min_pts) if pts.size else None
        if fit is None:
            return None
        m, b0, rmse = fit
        if abs(m) > 0.25 or rmse > max(8.0, out_w * 0.025):
            return None
        x0 = b0
        x1 = m * float(out_h - 1) + b0
        if side == "left":
            if min(x0, x1) < -out_w * 0.04 or max(x0, x1) > out_w * 0.28:
                return None
        else:
            if min(x0, x1) < out_w * 0.72 or max(x0, x1) > out_w * 1.04:
                return None
        return 1.0, -m, -b0

    y_top = float(top)
    y_bottom = float(out_h - 1 - bottom)
    x_left = float(left)
    x_right = float(out_w - 1 - right)
    lines = [
        (0.0, -1.0, y_top),
        (1.0, 0.0, -x_right),
        (0.0, -1.0, y_bottom),
    ...
    linefit_sides: list[str] = []
    for idx, side, fit in (
        (0, "top", _fit_horizontal_edge("top")),
        (1, "right", _fit_vertical_edge("right")),
        (2, "bottom", _fit_horizontal_edge("bottom")),
        (3, "left", _fit_vertical_edge("left")),
    ):
        if fit is not None:
            lines[idx] = fit
            linefit_sides.append(side)

    def _linefit_rect_quad() -> np.ndarray | None:
        if not linefit_sides:
            return None
        line_top, line_right, line_bot, line_left = lines
        p_tl = _intersect_lines(line_top, line_left)
        p_tr = _intersect_lines(line_top, line_right)
        ...
        if any(p is None for p in (p_tl, p_tr, p_br, p_bl)):
            return None
        rect_quad = np.stack([p_tl, p_tr, p_br, p_bl], axis=0).astype(np.float32)
        if not np.all(np.isfinite(rect_quad)):
            return None
        margin = max(6.0, min(out_w, out_h) * 0.03)
        xs = rect_quad[:, 0]
        ys = rect_quad[:, 1]
        if float(xs.min()) < -margin or float(xs.max()) > out_w - 1 + margin:
            return None
        if float(ys.min()) < -margin or float(ys.max()) > out_h - 1 + margin:
            return None
        area = float(abs(cv2.contourArea(rect_quad)))
        full_area = float(max(1, out_w * out_h))
        if area < full_area * 0.62 or area > full_area * 1.04:
            return None
        ordered = _order_quad(rect_quad)
        tl2, tr2, br2, bl2 = ordered
        min_w = min(float(np.linalg.norm(tr2 - tl2)), float(np.linalg.norm(br2 - bl2)))
        min_h = min(float(np.linalg.norm(bl2 - tl2)), float(np.linalg.norm(br2 - tr2)))
        if min_w < out_w * 0.68 or min_h < out_h * 0.68:
            return None
        return ordered

    rect_quad = _linefit_rect_quad()
    refine_kind = "linefit_applied" if rect_quad is not None else "applied"
    if rect_quad is None:
        ...
        rect_quad = np.array([[x0, y0], [x1, y0], [x1, y1], [x0, y1]], dtype=np.float32)
    else:
        full_rect = np.array([[0, 0], [out_w - 1, 0], [out_w - 1, out_h - 1], [0, out_h - 1]], dtype=np.float32)
        if max(left, top, right, bottom) <= 0 and float(np.max(np.linalg.norm(rect_quad - full_rect, axis=1))) <= 1.0:
            return quad, {
                "border_refine": "none",
                "border_thresholds": [round(l_min, 1), round(s_max, 1), round(c_max, 1)],
            }
    ...
    refined = cv2.perspectiveTransform(rect_quad.reshape(1, 4, 2), Minv)[0]
    info = {
        "border_refine": refine_kind,
        "border_trims_rect": [int(left), int(top), int(right), int(bottom)],
        "border_thresholds": [round(l_min, 1), round(s_max, 1), round(c_max, 1)],
    }
    if linefit_sides and refine_kind == "linefit_applied":
        info["border_linefit_sides"] = linefit_sides
    return _order_quad(refined), info
...
def segment_black_seeds(img_bgr: np.ndarray, all_mask: np.ndarray, paper_mask: np.ndarray):
    ...
    # When a true black seed touches a neighboring peeled seed, dilation can
    # leak into the neighbor through a warm, light-brown edge. Trim only that
    # boundary; keep warm highlights inside genuinely black hulls.
...
def collect_images(input_path: Path, recursive: bool) -> list[Path]:
    ...
    for path in iterator:
        if not path.is_file() or path.suffix.lower() not in IMAGE_EXTS or path.name.startswith("."):
            continue
        rel_parts = path.relative_to(input_path).parts[:-1]
        if any(part.lower().startswith("output") for part in rel_parts):
            continue
        images.append(path)
...
def parse_percent(value: str | None) -> float | None:
    ...
    try:
        ...
        return None
    return number / 100.0 if is_percent or abs(number) > 1.0 else number
...
def update_results_table(table_path: Path, results: list[dict], backup: bool = True) -> Path | None:
    if not results:
        return None

    rows: list[dict[str, str]] = []
    fieldnames = list(RESULTS_TABLE_COLUMNS)
    if table_path.exists():
        with table_path.open("r", encoding="utf-8-sig", newline="") as fh:
            reader = csv.DictReader(fh)
            existing_fields = reader.fieldnames or []
            fieldnames = list(existing_fields)
            for required in RESULTS_TABLE_COLUMNS:
    ...
    for result in results:
        ...
        if row is None:
            ...
            rows.append(row)
            by_name[image_name] = row
        seed_ratio = float(result["black_to_all_ratio"])
        lab_ratio = parse_percent(row.get("Lab Result"))
        row["SeedDetect Result"] = format_percent(seed_ratio)
        row["delta"] = format_percent(seed_ratio - lab_ratio) if lab_ratio is not None else ""

    backup_path = None
    if backup and table_path.exists():
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        backup_path = table_path.with_name(f"{table_path.stem}.backup_{stamp}{table_path.suffix}")
...
def main() -> None:
    ...
    parser.add_argument("--results-table", help="CSV table to update with SeedDetect Result and delta")
    parser.add_argument("--no-table-backup", action="store_true", help="Do not create a timestamped backup before updating --results-table")
    ...
    if input_path.is_dir() and output_dir != input_path and output_dir.is_relative_to(input_path):
        images = [path for path in images if not path.resolve().is_relative_to(output_dir)]
    if not images:
    ...
    table_backup_path = None
    if args.results_table:
        table_backup_path = update_results_table(Path(args.results_table).resolve(), results, backup=not args.no_table_backup)
    ...
    if args.results_table:
        print(f"table:   {Path(args.results_table).resolve()}")
        if table_backup_path is not None:
            print(f"backup:  {table_backup_path}")
